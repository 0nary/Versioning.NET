...
{
    public class GetVersionIncrementHandlerTests
    {
        ...
        public async Task Handler_ExitsWhen_IncrementIsNone()
        {
            ...
            var logger = new NullLogger<GetVersionIncrementHandler>();
            ...
            var sut = new GetVersionIncrementHandler(mediator.Object, gitVersioningService.Object, assemblyVersioningService.Object, logger);

            // Act
            await sut.Handle(new GetVersionIncrementQuery(), CancellationToken.None);
            ...
            assemblyVersioningService.Verify(x => x.GetLatestAssemblyVersion(It.IsAny<string>(), It.IsAny<SearchOption>()), Times.Once);
        ...
        {
            // Arrange
            var request = new GetVersionIncrementQuery
            {
                GitDirectory = "C:\\Temp",
            ...
            // Assert
            assemblyVersioningService.Verify(x => x.GetLatestAssemblyVersion(request.TargetDirectory, request.SearchOption), Times.Once);
        }
        ...
        public async Task Handler_CallsDependencies()
        {
            ...
            // Assert
            mediator.Verify(x => x.Send(It.IsAny<GetCommitVersionInfosQuery>(), CancellationToken.None), Times.Once);
            mediator.Verify(x => x.Send(It.IsAny<IncrementVersionByWithGitIntegrationCommand>(), CancellationToken.None), Times.Never);
            mediator.Verify(x => x.Send(It.IsAny<IncrementAssemblyVersionCommand>(), CancellationToken.None), Times.Never);
            assemblyVersioningService.Verify(x => x.GetLatestAssemblyVersion(request.TargetDirectory, request.SearchOption), Times.Once);
